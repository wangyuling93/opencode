...
constructed error; both reject callers with `{ type, message, data? }`. RPC IDs
are independent of plugin IDs. One plugin can implement several RPCs, and later registrations override earlier ones
