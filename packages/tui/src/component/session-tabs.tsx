...
function VerticalSessionTabs(props: {
  ...
  const stride = () => (compact() ? 2 : transparent() ? 5 : 3)
  ...
  createEffect(() => {
    ...
    const height = compact() ? 3 : newTab() ? 1 : transparent() ? 4 : 2
        ...
        <box flexShrink={0} flexDirection="column" gap={1} paddingY={compact() ? 1 : 0}>
          <For each={items()}>
            {(tab, index) => {
              ...
              const restingTitleWidth = () =>
                Math.max(1, width() - prefixWidth() - 1 - (!compact() && transparent() ? 2 : 0))
              ...
              return (
                <box
                  ...
                  height={compact() ? 1 : transparent() ? 4 : 2}
                  width="100%"
                  position="relative"
                  flexDirection="column"
                  backgroundColor={background()}
                  border={!compact() && transparent() ? true : undefined}
                  borderColor={!compact() && transparent() ? foreground() : undefined}
                  onMouseOver={(event) => {
                    setHoverY(event.y)
